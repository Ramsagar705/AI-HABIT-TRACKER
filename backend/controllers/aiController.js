...
${ctx.perHabit
  .map(
    (h) =>
      `- ${h.name} (${h.category}, ${h.frequency}): completed ${h.completedDays} days, target ${h.targetDays}`
  )
  .join("\n")}

Please write a personalized weekly report.
`;
...
`;
    ...
    if (!suggestions.length) {
      suggestions = [
        ...
        {
          name: "2 minutes mindful breathing",
          description:
  ...
  try {
    ...
    const context = habits
      .map((h) => {
        ...
        return `${h.name} (${h.category}) : ${hLogs.length}/30 completions`;
      })
      .join("\n");
...
export const morningMotivation = async (req, res) => {
  try {
    ...
    if (!habits.length) {
      return res.json({
        content:
          "Good morning! Add your first habit today and let's get started.",
      });
    }
